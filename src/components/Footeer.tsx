...
const Footeer: React.FC = () => {
  ...
  // Define the color palette as constants for easier management within the file
  const colors = {
    deepNavy: {
      900: '#1A202C', // Very dark blue/gray for background
      800: '#2D3748', // Slightly lighter for elements like social buttons, input fields
      700: '#4A5568', // For borders and separators
    },
    softGrey: {
      300: '#CBD5E0', // Lighter grey for general text
      400: '#A0AEC0', // Even lighter grey for placeholder text, copyright
      600: '#718096', // For input borders
    },
    terracotta: {
      400: '#E98569', // Primary accent color (lighter shade)
      500: '#D76B4D', // Primary accent color (main shade for logo)
    },
    warmCoral: {
      400: '#FF6F61', // Gradient end for accent, also for icons/hover states
      500: '#E5533D', // Button background, gradient start
      600: '#CC4A35', // Button hover
    },
  };
  ...
  return (
    <footer className={`bg-[${colors.deepNavy[900]}] text-white relative overflow-hidden`}>
      {/* Background Pattern */}
      <div className="absolute inset-0 opacity-5">
        {/* The SVG fill color is updated using the specific hex value */}
        <div className={`absolute inset-0 bg-[url('data:image/svg+xml,%3Csvg width=%2260%22 height=%2260%22 viewBox=%220 0 60 60%22 xmlns=%22http://www.w3.org/2000/svg%22%3E%3Cg fill=%22none%22 fill-rule=%22evenodd%22%3E%3Cg fill=%22${colors.terracotta[400].replace('#', '%23')}%22 fill-opacity=%220.4%22%3E%3Ccircle cx=%2230%22 cy=%2230%22 r=%222%22/%3E%3C/g%3E%3C/g%3E%3C/svg%3E')]`} />
      </div>
      ...
      <div className="relative z-10">
        ...
        <motion.div
          ...
          transition={{ duration: 0.8 }}
          viewport={{ once: true }}
          className={`border-b border-[${colors.deepNavy[700]}] py-16`}
        >
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="grid lg:grid-cols-2 gap-8 items-center">
              <div>
                <h3 className="text-3xl lg:text-4xl font-bold mb-4">
                  Stay ahead with{' '}
                  <span className={`text-transparent bg-clip-text bg-gradient-to-r from-[${colors.terracotta[400]}] to-[${colors.warmCoral[400]}]`}>
                    expert insights
                  </span>
                </h3>
                <p className={`text-xl text-[${colors.softGrey[300]}] leading-relaxed`}>
                  Get weekly tips, industry trends, and exclusive training resources delivered to your inbox.
                </p>
              </div>
              ...
              >
                <input
                  type="email"
                  placeholder="Enter your email address"
                  className={`flex-1 px-6 py-4 bg-[${colors.deepNavy[800]}] border border-[${colors.softGrey[600]}] rounded-xl text-white placeholder-[${colors.softGrey[400]}] focus:ring-2 focus:ring-[${colors.warmCoral[500]}] focus:border-transparent transition-all duration-200`}
                />
                <motion.button
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                  className={`bg-[${colors.warmCoral[500]}] hover:bg-[${colors.warmCoral[600]}] text-white px-8 py-4 rounded-xl font-semibold whitespace-nowrap transition-all duration-300`}
                >
                  Subscribe
                </motion.button>
              </motion.div>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="grid lg:grid-cols-5 gap-12">
              ...
              >
                {/* Logo */}
                <div className="flex items-center space-x-3 mb-6">
                  <div className={`w-12 h-12 bg-gradient-to-br from-[${colors.terracotta[500]}] to-[${colors.warmCoral[500]}] rounded-xl flex items-center justify-center`}>
                    <span className="text-white font-bold text-xl">U</span>
                  </div>
                  <div>
                    <h3 className="text-2xl font-bold">Upwise</h3>
                    {/* <p className={`text-sm text-[${colors.softGrey[400]}]`}>by Ritu Arora</p> */}
                  </div>
                </div>

                <p className={`text-[${colors.softGrey[300]}] leading-relaxed mb-8`}>
                  Empowering professionals to grow smarter, present sharper, and perform better through expert-led corporate training and professional development programs.
                </p>
                    ...
                    {section.links.map((link) => (
                      <li key={link.name}>
                        <motion.a
                          href={link.href}
                          whileHover={{ x: 5 }}
                          className={`text-[${colors.softGrey[300]}] hover:text-[${colors.warmCoral[400]}] transition-all duration-200`}
                        >
                          {link.name}
                        </motion.a>
                      </li>
        ...
        <motion.div
          ...
          transition={{ duration: 0.8 }}
          viewport={{ once: true }}
          className={`border-t border-[${colors.deepNavy[700]}] py-8`}
        >
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex flex-col lg:flex-row items-center justify-between gap-6">
              {/* Copyright */}
              <div className={`text-[${colors.softGrey[400]}] text-sm`}>
                © {currentYear} Upwise by Ritu Arora. All rights reserved.
              </div>

              {/* Legal Links */}
              <div className="flex items-center gap-6 text-sm">
                <a href="#" className={`text-[${colors.softGrey[400]}] hover:text-[${colors.warmCoral[400]}] transition-colors duration-200`}>
                  Privacy Policy
                </a>
                <a href="#" className={`text-[${colors.softGrey[400]}] hover:text-[${colors.warmCoral[400]}] transition-colors duration-200`}>
                  Terms of Service
              ...
              <div className="flex items-center gap-4">
                {socialLinks.map((social) => (
                  <motion.a
                    ...
                    whileHover={{ scale: 1.1, y: -2 }}
                    whileTap={{ scale: 0.9 }}
                    className={`w-10 h-10 bg-[${colors.deepNavy[800]}] hover:bg-[${colors.warmCoral[500]}] rounded-lg flex items-center justify-center text-[${colors.softGrey[400]}] hover:text-white transition-all duration-300`}
                  >
                    <social.icon size={20} />
                  </motion.a>
                ))}
